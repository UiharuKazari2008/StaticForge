...
| `agent_session_command` | Loopback `/agent/session/*` or MCP to **this key's** bound Studio tab | `requestId` + `data.command` (`open_image` / `open_viewer` / `apply_studio` / `get_state` / `get_windows` / `get_physics` / `run_client_js` / `inspect_elements` / `client_prepare_update` / `client_restart` / `client_update`) and optional `data.actorName` — `public/scripts/comp/agentClientBridge.js`. `get_windows` lists open Lumen / Glancewell / Grimoire / gallery / Studio with current data. `open_viewer` takes `target` (`lumen` \| `glancewell`) and `filenames`. `apply_studio` includes sibling `autoApply` (default true) and `autoGenerate` (default false). Silent apply skips autofill. `run_client_js` takes `script` (awaits a Promise) and replies `{ result }` or `{ error }`. `inspect_elements` takes `selectors` plus optional `html` / `style` / `text` / `visible` / `box` / `attrs` / `styleAllowlist`. `client_prepare_update` follows desktop Update (`refresh-cache`): CSS/apply-safe replies `appliedWithoutRestart`; JS/HTML replies `readyForRestart`. `client_restart` reloads; the server waits for reattach. |
...
| `mcp_open_viewer` | MCP `open_in_lumen` / `open_in_glancewell` when no bind (never after a bound `open_viewer` timeout) | `data.target`, `data.filenames[]` — `public/scripts/comp/mcpActivityClient.js` |
