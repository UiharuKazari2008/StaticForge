...
| `agent_session_command` | Loopback `/agent/session/*` or MCP to **this key's** bound Studio tab | `requestId` + `data.command` (`open_image` / `open_viewer` / `apply_studio` / `get_state` / `get_windows` / `get_physics` / `run_client_js` / `inspect_elements`) and optional `data.actorName` — `public/scripts/comp/agentClientBridge.js`. `get_windows` lists open Lumen / Glancewell / Grimoire / gallery / Studio with current data. `open_viewer` takes `target` (`lumen` \| `glancewell`) and `filenames`. `apply_studio` includes sibling `autoApply` (default true) and `autoGenerate` (default false). Silent apply skips autofill. `run_client_js` takes `script` and replies `{ result }` or `{ error }`. `inspect_elements` takes `selectors` plus optional `html` / `style` / `styleAllowlist`. |
...
| `mcp_open_viewer` | MCP `open_in_lumen` / `open_in_glancewell` / `open_in_prism` when no bind | `data.target`, `data.filenames[]` — `public/scripts/comp/mcpActivityClient.js` |
