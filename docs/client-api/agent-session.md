...
| `GET /agent/session/state` | — | Snapshot: `workspaceId`, open `filename` (null if ungenerated / no image), `model`, bound `clientId`, plus `change` (Studio editor as Change-JSON v1), `dynamicGeneration` (Studio toggles plus `resolved` time/weather/season/location; `directorApi: "noop"` — Grok Web compiles), `director`, `scopes`, and `vfsPathUuid` when the key has `vfs`. No image required. Ivory rewrites `change` and `POST /agent/session/studio`. |
