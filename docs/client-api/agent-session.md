...
| MCP `open_in_lumen` / `open_in_glancewell` / `open_in_prism` | `{ "target", "filenames" }` | Bound `open_viewer` or push `mcp_open_viewer` → Lumen / Glancewell / Prism |
...
| MCP `get_open_windows` | Bound `get_windows` | Open windows on that tab: Lumen filename, Glancewell current + nearby files, Grimoire address + page text, gallery selected filenames, Studio filename/model, Prism compare set. |
