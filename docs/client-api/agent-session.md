...
| MCP `open_in_lumen` / `open_in_glancewell` | `{ "target", "filenames" }` | Bound `open_viewer`, else push `mcp_open_viewer` (not both) → Lumen / Glancewell |
...
| `POST /agent/session/prepare-update` | — | Silent `client_prepare_update`: same as desktop **Update** (`refresh-cache`). CSS/apply-safe applies in place (`appliedWithoutRestart`). JS/HTML waits until `readyForRestart`. `alreadyCurrent` if nothing to apply. No dialog. MCP `update_client`. |
| `POST /agent/session/restart` | — | Silent `client_restart`, then wait until the same session reconnects and this key rebinds. MCP `restart_client`. |
| `POST /agent/session/js` | `{ "script" }` | Bound `run_client_js`. Awaits a returned Promise. |
| `POST /agent/session/inspect` | `{ "selectors" }` plus optional `html` / `style` / `text` / `visible` / `box` / `attrs` / `styleAllowlist` | Bound `inspect_elements`. |
...
| `autoGenerate` | `false` | After a **successful apply**, click the bound tab's existing Generate button (`#manualGenerateBtn` → form submit → `handleManualGeneration` / `generate_image`). Uses Yukimi's bound session, not a server-side generate. MCP apply returns `jobId` + `filenameBefore` (and `blockedBy: printsReview` if Prints review still blocks Generate). Default `n` is 1. Then `await_generation_job` or `get_generated_image` `since=apply` — do not treat the pre-apply filename as the new print. |
