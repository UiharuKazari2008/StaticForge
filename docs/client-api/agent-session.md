...
| `POST /agent/session/prepare-update` | — | Silent `client_prepare_update`: check/download SW assets; wait until `readyForRestart` or `alreadyCurrent`. No dialog. MCP `update_client`. |
