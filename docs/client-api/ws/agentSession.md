...
| Type | When | Data |
|------|------|------|
| `agent_session_command` | Loopback REST / MCP drive of this key's bound tab | `requestId` + `data.command` (`open_image` / `apply_studio` / `get_state` / `get_windows` / `get_editor` / `get_physics` / `run_client_js` / `inspect_elements` / `client_update`) and optional `data.actorName` (application token name for Remote Access tray copy). `get_windows` returns open Lumen / Glancewell / Grimoire / gallery / Studio / Prism with current data. `open_viewer` takes `target` (`lumen` \| `glancewell` \| `prism`) and `filenames`. `apply_studio` also carries sibling `autoApply` (default true) and `autoGenerate` (default false). Silent apply sets `skipAutofill` on field writes. `get_physics` returns the tab's dynamic-button config; the server compiles location/tod/date/weather/season. `run_client_js` takes `script` and replies `{ result }` or `{ error }`. `inspect_elements` takes `selectors` plus optional `html` / `style` / `styleAllowlist` and replies html and/or computed style per selector. `client_update` shows the mandatory 15s Client Update dialog; Cancel aborts, 0 applies then restarts that tab. `POST /agent/broadcast` `restart: true` reuses that same dialog on every connected tab (not this bound command). |
