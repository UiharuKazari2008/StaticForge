...
| Type | When | Data |
|------|------|------|
| `agent_session_command` | Loopback REST / MCP drive of this key's bound tab | `requestId` + `data.command` (`open_image` / `apply_studio` / `get_state` / `get_windows` / `get_editor` / `get_physics` / `run_client_js` / `inspect_elements` / `client_update` / `client_prepare_update` / `client_restart`) and optional `data.actorName` (application token name for Remote Access tray copy). `get_windows` returns open Lumen / Glancewell / Grimoire / gallery / Studio with current data. `open_viewer` takes `target` (`lumen` \| `glancewell`) and `filenames`. `apply_studio` also carries sibling `autoApply` (default true) and `autoGenerate` (default false). Silent apply sets `skipAutofill` on field writes. `get_physics` returns the tab's dynamic-button config; the server compiles location/tod/date/weather/season. `run_client_js` takes `script` (awaits a Promise) and replies `{ result }` or `{ error }`. `inspect_elements` takes `selectors` plus optional `html` / `style` / `text` / `visible` / `box` / `attrs` / `styleAllowlist` and replies tag/id/text/visible plus optional html/style/box/attrs per selector. `client_prepare_update` follows desktop Update (`refresh-cache`): CSS/apply-safe replies `appliedWithoutRestart`; JS/HTML replies `readyForRestart`; nothing to apply is `alreadyCurrent`. `client_restart` replies `{ restarting: true }` then reloads; the server waits for reattach. `client_offer_testing` shows the existing confirmation dialog on a non-primary tab; `client_dismiss_testing_offer` clicks No / closes it. Several tabs: nearest auto-binds; no response (15s) keeps the primary and dismisses others; Yes rebinds to that tab. Same-tab WebSocket reconnect resumes the stored `clientId` (`?agentClientId=` on the socket URL; echoed on the `connection` welcome) and does not re-offer. `client_update` shows the mandatory 15s Client Update dialog; Cancel aborts, 0 applies then restarts that tab. `POST /agent/broadcast` `restart: true` reuses that same dialog on every connected tab (not this bound command). |
