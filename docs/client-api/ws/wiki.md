...
Renders the Apocrypha zine interior HTML for Grimoire (`tagWikiSearchModal` / Apocrypha DSAP). Live issue by default; pass a slug to read an archived day. Session WS always includes Grim. The public HTTP reader (`/{apocryphaPathUuid}`, including `/archive/<slug>`) shows Grim to logged-in cookies **or** local IPs via `config.apocrypha` (trusted-proxy XFF — see [README · Apocrypha public Grim](../README.md#apocrypha-public-grim-http)). Home WAN hairpin is not local.
