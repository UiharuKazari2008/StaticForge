...
| Public MCP / Grok connector | `POST /{mcpPathUuid}` Streamable HTTP (`generate_image`, `get_generated_image`, `get_images`, `get_workspaces`, `list_clients`, `bind_session`, `apply_studio_changes`) | same packets as `/agent/packet` / `agent_session_command` | — | Facade over existing `/agent` + WS. Bearer `sfapp_`. Unknown UA bypass + capture in `application_user_agents_seen`. No new chrome. |
