...
| Public MCP / Grok connector | `POST /{mcpPathUuid}` Streamable HTTP (generate/gallery/workspace/studio, autofill, tag+static wiki, presets, references, OmegaSearch, notes) | same packets as `/agent/packet` / `agent_session_command` | — | Facade over existing `/agent` + WS. Bearer `sfapp_`. Unknown UA bypass + capture in `application_user_agents_seen`. No new chrome. |
