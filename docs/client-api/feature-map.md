...
| Public MCP / Grok connector | `POST /{mcpPathUuid}` Streamable HTTP plus OAuth discovery and `/{mcpPathUuid}/oauth/*` | same packets as `/agent/packet` / `agent_session_command` (generate/upscale/expand, gallery/workspace/studio, autofill, tag+static wiki, presets, references, OmegaSearch, notes) | — | Facade over existing `/agent` + WS. Auth is Bearer `sfapp_` or OAuth `mcoat_`; static-key unknown UA bypass + capture in `application_user_agents_seen`. Consent HTML only under the UUID OAuth path. |
