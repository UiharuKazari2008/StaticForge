...
| Agent session drive | `POST /agent/session/open-image`, `POST /agent/session/studio`, `POST /agent/session/update`, `GET /agent/session/state` | `agent_session_result` | `agent_session_command` | Opens Studio / applies change JSON (`autoApply` default true, `autoGenerate` default false) / bound-tab Client Update dialog (15s cancel, then apply+restart) / state snapshot (`change` = ungenerated editor as Change-JSON v1) on the bound tab |
