...
| Public MCP / Grok connector | `POST /{mcpPathUuid}` Streamable HTTP (generate/upscale/expand/staged, gallery delete/scrap/favorite, Lumen/Glancewell, compare, theme eval, VFS/desktop, studio, autofill, wiki, presets, notes) | same packets as `/agent/packet` / `agent_session_command`; pushes `mcp_activity`, `mcp_open_viewer`, `gallery_updated` | Tray `#mcpActivityIndicator` (2 min hover), Spectator MCP list, Properties `forgedata-badge` **MCP** | Facade over existing `/agent` + WS. Bearer `sfapp_`. MCP generate writes `forge_data.mcp_generated` and notifies the open gallery. |
