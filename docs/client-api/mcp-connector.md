...
|------|------|-------|-------|
| `generate_image` | Server generate on the **shared generation FIFO** (Studio Generate / preset / reroll use the same stack; 8–20s gap after each job). Default stalls until filename + Grok webp (tool channel) + `dest_path` / `bytes` / `mime` / short-lived `url`. Pass `dest_path` (`artifacts/<file>.webp`); `curl` `url` into `/home/workdir/artifacts` if missing, then `render_file`. `async: true` returns `jobId` immediately and remembers `dest_path` for `await_generation_job`. Full Studio settings as top-level keys or `params`. `n` is print count 2–8. Writes `forge_data.mcp_generated`. Always pass `workspace`: the folder they named, or where the source / this job's gens already live. Do not copy the bound Studio tab just because it is focused. Omitting `workspace` is unsafe (server may fall back to that tab or `default`). Paid Anlas/Opus (upscale, expand, large/xlarge/wallpaper) requires `userApprovedPaidRequest` (alias `allow_paid`) or the tool bounces before FIFO. `dynamicGeneration.enabled: false` or omit does not compile and does not 500. Unintegrated `dynamicGeneration` returns `needsIntegration` + resolved and does not enqueue. | `generation` | no |
| `get_generation_job` | Poll `jobId` from `async` generate. Status/position while queued; same image + `dest_path` payload when done. | `generation` | no |
| `await_generation_job` | Block on `jobId` until complete, then return filename + Grok webp + `dest_path`. Same `dest_path` as `generate_image`. | `generation` | no |
| `get_generated_image` | Metadata + Grok webp (tool channel) + `dest_path` / `bytes` / `mime` / `url`. Filename, seed, or omit for latest. Pass the same `workspace` as the job / source gens (not the focused Studio tab). Omit means `default`. After `apply_studio_changes` `autoGenerate`, call this with `dest_path` then `render_file`. | `gallery` | no |
| `get_workspaces` | `workspace_list` — id, name, and `nicknames[]`. Pass id, display name, or a nickname (`the lab`, `prego`) as `workspace` on generate / get / omegasearch | `workspace` | no |
| `get_session_state` | Session snapshot. Default `view=live` (clients, windows, Studio, `remoteAccess`). After apply/get the bound tab stores a checkpoint; later live checks return only the Studio delta (`studio.diff`). `view=catalog` slim settings. `view=full` live + slim catalog + promptGuide/NAX/memory pointers. Full per-model quality/UC strings are on `get_studio_state.settings` / `tools/list`. `hasClients` false or `studioReachable` false → `generate_image`. Grimoire window text is truncated; full text via `get_open_windows`. | `generation` | if clients |
| `get_studio_state` | Bound `get_state` for **this application key** (stays bound 15 min / until tray Unbind). One tab auto-binds. Several tabs: `needsClientChoice` + clients most recently used first — ask, then `bind_session`. Later calls return only the delta unless `full: true`. Plus `settings` on a full snapshot, `dynamicGeneration` (toggles + `resolved` time/weather/season/location; Director compile is nooped), `director`, and `mustAct` when those fields are present. Prefer `get_session_state`. | `generation` | yes |
...
| `update_client` | Silent SW update on the bound tab, same as desktop context-menu **Update** (`refresh-cache`): `refreshServerCache` then download. CSS/apply-safe returns `appliedWithoutRestart` (stylesheets swapped; do **not** `restart_client`). JS/HTML returns `readyForRestart`. `alreadyCurrent` means nothing to apply. No 15s dialog. After client-asset edits run `scripts/notify-service-worker-update.sh` first. | `generation` | yes |
| `restart_client` | Restart the bound tab and wait until that session reconnects, this key rebinds, and `get_state` answers. Call only when `update_client` returned `readyForRestart`. Skip when `appliedWithoutRestart` or `alreadyCurrent`. Then inspect/js. | `generation` | yes |
...
| `get_character_card` | One character card: wiki markdown + request expander + Studio box snapshot (`action: replace` + `index`) + best NAX CHARA `item.prompt`. `name` (aliases `tag` / `query`); optional `franchise` / `model`. Wiki miss or timeout still returns NAX. Empty wiki → do not invent appearance. | `wiki` (also listed for `autofill` keys) | no |
...
| `resolve_lookback` | Resolve a pasted Copy Lookback markdown link (`[label](dsap://lookback/…)`) or raw `dsap://lookback/…` to that item’s metadata. Gallery images and Reference Manager refs also return a Grok-sized webp. Do not mint lookback URIs. | `search` / core | no |
...
| `open_in_lumen` / `open_in_glancewell` | Bound `open_viewer`, else `mcp_open_viewer` (not both) | `gallery` | client |
