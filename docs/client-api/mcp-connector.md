...
| `get_generated_image` | Metadata + Grok webp (tool channel) + `dest_path` / `bytes` / `mime` / `url`. Filename, seed, or omit for latest. `workspace` default is `default`. After `apply_studio_changes` `autoGenerate`, call this with `dest_path` then `render_file`. | `gallery` | no |
