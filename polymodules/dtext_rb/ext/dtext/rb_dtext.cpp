...
static VALUE c_parse(VALUE self, VALUE input, VALUE base_url, VALUE domain, VALUE internal_domains, VALUE emojis, VALUE f_inline, VALUE f_disable_mentions, VALUE f_media_embeds, VALUE source) {
  if (NIL_P(input)) {
    return Qnil;
  }
  ...
  Check_Type(emojis, T_ARRAY);
  options.is_allowed_emoji = [&emojis](const std::string_view name) {
    VALUE rb_name = rb_str_new(name.begin(), name.size());
    rb_funcall(rb_name, rb_intern("downcase!"), 0);

    return RTEST(rb_ary_includes(emojis, rb_name));
  };

  // Set source if provided
  if (!NIL_P(source)) {
    options.source = StringValueCStr(source);
  } else if (!options.base_url.empty()) {
    // Auto-detect source from base_url
    if (options.base_url.find("e621.net") != std::string::npos) {
      options.source = "e621";
    } else {
      options.source = "danbooru";
    }
  }
