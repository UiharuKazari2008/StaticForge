...
action in_color { dstack_is_open(INLINE_COLOR) }
...
# - [section] - no title, not expanded
# - [section=title] - with title, not expanded
# - [section,expanded] - no title, expanded
# - [section,expanded=title] - expanded first, then title (most common in e621)
# - [section=title,expanded] - with title, then expanded
# - [section title="title"] - using tag_attributes
open_section_plain = '[section]'i | '<section>'i;
open_section_expanded_plain = '[section'i ws* ',' ws* 'expanded' ws* ']';
open_section_expanded_title = '[section'i ws* ',' ws* 'expanded' ws* '=' ws* ((nonnewline - ']')* >mark_a1 %mark_a2) ']';
open_section_shorthand = '[section'i ws* '=' ws* ((nonnewline - ',' - ']')* >mark_a1 %mark_a2) (ws* ',' ws* 'expanded' ws*)? ']' | '[section'i ws* '=' ws* ((nonnewline - ']')* >mark_a1 %mark_a2) ws* ',' ws* 'expanded' ws* ']';
open_section_attrs = '[section'i tag_attributes :>> ']' | '<section'i tag_attributes :>> '>';
open_section = open_section_plain | open_section_expanded_plain | open_section_expanded_title | open_section_shorthand | open_section_attrs;
...
open_s = '[s]'i | '<s>'i;
open_u = '[u]'i | '<u>'i;
open_color = '[color'i ws* '=' ws* ((nonnewline - ']')* >mark_a1 %mark_a2) ']' | '<color'i ws* '=' ws* ((nonnewline - '>')* >mark_a1 %mark_a2) '>';
...
basic_inline := |*
  ...
  open_color => {
    if (a1 != nullptr && a2 != nullptr && a2 > a1) {
      std::string color_value(a1, a2 - a1);
      // Trim whitespace
      while (!color_value.empty() && (color_value.front() == ' ' || color_value.front() == '\t')) {
        color_value.erase(0, 1);
      }
      while (!color_value.empty() && (color_value.back() == ' ' || color_value.back() == '\t')) {
        color_value.pop_back();
      }
      // Validate color value (hex, named color, rgb, etc.)
      // For now, accept any value and let CSS handle validation
      std::string html = "<span style=\"color: ";
      html += color_value;
      html += "\">";
      dstack_open_element(INLINE_COLOR, html.c_str());
    } else {
      // Fallback if no color value
      dstack_open_element(INLINE_COLOR, "<span>");
    }
  };
  close_color => { dstack_close_element(INLINE_COLOR, { ts, te }); };
...
inline := |*
  ...
  newline (code_fence | open_code | open_code_lang | open_nodtext | open_table | open_expand | aliased_expand | open_section | hr | header | header_with_id | tag_request_embed | media_embed) => {
    dstack_close_leaf_blocks();
    fexec ts;
    fret;
  };
...
main := |*
  ...
  open_section_plain space* => {
    dstack_close_leaf_blocks();
    // Generate unique section ID using position in input stream
    std::string section_id = "section-" + std::to_string(p - input.data());
    
    std::string title = "Section";
    bool is_expanded = false;
    
    dstack_open_element(BLOCK_SECTION, "<div class=\"tag-wiki-section\">");
    append_block("<div class=\"tag-wiki-section-toggle\" data-target=\"");
    append_block(section_id);
    append_block("\"><i class=\"fas ");
    append_block(is_expanded ? "fa-chevron-down" : "fa-chevron-right");
    append_block("\"></i><span>");
    append_block_html_escaped(title);
    append_block("</span></div><div id=\"");
    append_block(section_id);
    append_block("\" class=\"tag-wiki-section-content");
    if (!is_expanded) {
      append_block(" hidden");
    }
    append_block("\">");
  ...
  open_section_shorthand space* => {
    ...
    // Extract title from shorthand format [section=title] or [section=title,expanded]
    if (a1 != nullptr && a2 != nullptr && a2 > a1) {
      std::string extracted_title(a1, a2 - a1);
      if (!extracted_title.empty() && extracted_title.find('/') == std::string::npos) {
    ...
    // Check if pattern includes ",expanded" by examining the full match
    const char* match_start = ts;
    const char* match_end = te;
    std::string match_str(match_start, match_end - match_start);
    if (match_str.find(",expanded") != std::string::npos || match_str.find(", expanded") != std::string::npos) {
      is_expanded = true;
    }
  ...
  open_section_expanded_plain space* => {
    dstack_close_leaf_blocks();
    // Generate unique section ID using position in input stream
    std::string section_id = "section-" + std::to_string(p - input.data());
    
    std::string title = "Section";
    bool is_expanded = true; // [section,expanded] is always expanded
  ...
  open_section_expanded_title space* => {
    dstack_close_leaf_blocks();
    // Generate unique section ID using position in input stream
    std::string section_id = "section-" + std::to_string(p - input.data());
    
    std::string title = "Section";
    bool is_expanded = true; // [section,expanded=title] is always expanded
    
    // Extract title from [section,expanded=title] format
    if (a1 != nullptr && a2 != nullptr && a2 > a1) {
      std::string extracted_title(a1, a2 - a1);
      if (!extracted_title.empty() && extracted_title.find('/') == std::string::npos) {
        title = extracted_title;
  ...
  open_section_attrs space* => {
    dstack_close_leaf_blocks();
    // Generate unique section ID using position in input stream
    std::string section_id = "section-" + std::to_string(p - input.data());
    
    // Extract title and expanded from tag_attributes
    std::string title = "Section";
    bool is_expanded = false;
    
    // Check tag_attributes for title and expanded
    auto title_it = tag_attributes.find("title");
    if (title_it != tag_attributes.end()) {
      title = std::string(title_it->second);
    }
    
    auto expanded_it = tag_attributes.find("expanded");
    if (expanded_it != tag_attributes.end()) {
      std::string expanded_val(expanded_it->second);
      is_expanded = (expanded_val == "true" || expanded_val == "1" || expanded_val == "yes");
    }
  ...
  space* (close_section when in_section) ws* => {
    dstack_close_until(BLOCK_SECTION);
  };
  ...
  any => {
    ...
    if (dstack.empty() || dstack_check(BLOCK_QUOTE) || dstack_check(BLOCK_SPOILER) || dstack_check(BLOCK_EXPAND) || dstack_check(BLOCK_SECTION) || dstack_check(BLOCK_MEDIA_GALLERY)) {
...
void StateMachine::append_wiki_link(const std::string_view prefix, const std::string_view tag, const std::string_view anchor, const std::string_view title, const std::string_view suffix) {
  ...
  // Check for file@thumb# or file@post# patterns - convert to embedded images
  // Format from create-tag-database.js: [[file@post12345]] or [[file@thumb12345]]
  // Files are stored at .cache/wiki_files/{type}{id}.jpg and served at /cache/wiki_files/{type}{id}.jpg
  if (normalized_tag.starts_with("file@thumb") || normalized_tag.starts_with("file@post")) {
    std::string image_path;
    std::string image_id;
    
    if (normalized_tag.starts_with("file@thumb")) {
      // Extract ID after "file@thumb" (10 chars)
      image_id = normalized_tag.substr(10);
      image_path = "/cache/wiki_files/thumb" + image_id + ".jpg";
    } else if (normalized_tag.starts_with("file@post")) {
      // Extract ID after "file@post" (9 chars)
      image_id = normalized_tag.substr(9);
      image_path = "/cache/wiki_files/post" + image_id + ".jpg";
    }
    
    // Output image tag
    append("<img src=\"");
    append_html_escaped(image_path);
    append("\" alt=\"");
    if (!title_string.empty()) {
      append_html_escaped(title_string);
    } else {
      append_html_escaped(normalized_tag);
    }
    append("\" class=\"wiki-embedded-image\"");
    
    // Add data attributes for reference
    append(" data-wiki-tag=\"");
    append_html_escaped(normalized_tag);
    append("\"");
    
    if (!image_id.empty()) {
      append(" data-image-id=\"");
      append_html_escaped(image_id);
      append("\"");
    }
  ...
  // Output tag-wiki-link class for JavaScript compatibility
  append("<a class=\"dtext-link tag-wiki-link");
  
  // Add source class if source is set
  if (!options.source.empty()) {
    append(" dtext-source-");
    append_html_escaped(options.source);
  }
  ...
  // Extract tag name for data-tag-name (convert normalized_tag back to readable format)
  std::string tag_name_display = normalized_tag;
  std::replace(tag_name_display.begin(), tag_name_display.end(), '_', ' ');
  append(" data-tag-name=\"");
  append_html_escaped(tag_name_display);
  append("\"");
...
void StateMachine::dstack_rewind() {
  ...
  switch(element) {
    ...
    case BLOCK_SECTION: append_block("</div></div>"); break;
...
void StateMachine::dstack_close_leaf_blocks() {
  ...
  while (!dstack.empty() && !dstack_check(BLOCK_QUOTE) && !dstack_check(BLOCK_SPOILER) && !dstack_check(BLOCK_EXPAND) && !dstack_check(BLOCK_SECTION) && !dstack_check(BLOCK_TN) && !dstack_check(BLOCK_MEDIA_GALLERY)) {
    dstack_rewind();
  }
}
