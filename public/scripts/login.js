...
class LoginPage {
    ...
    updateAriaAttributes() {
        ...
        const ariaLabel = isMinimized
            ? `Show PIN pad${digitsEntered > 0 ? `, ${digitsEntered} digits entered` : ''}`
            : `Hide PIN pad${digitsEntered > 0 ? `, ${digitsEntered} digits entered` : ''}`;
    ...
    async handleLogin() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.setErrorMessage(data.message || 'Invalid PIN');
                this.showPinError();
                this.clearPin();
            }
        } catch (error) {
