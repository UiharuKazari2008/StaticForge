...
class LoginPage {
    ...
    togglePinPad(forceState) {
        ...
        if (typeof forceState === 'boolean') {
            if (forceState) {
                this.loginContainer.classList.add('minimize');
            } else {
    ...
    updateAriaAttributes() {
        ...
        const pinCount = this.rollingBuffer.length;
        const countText = pinCount > 0 ? `, ${pinCount} digit${pinCount === 1 ? '' : 's'} entered` : '';
    ...
    triggerButtonFeedback(keyOrAction) {
        let button;
        if (/^[0-9]$/.test(keyOrAction)) {
            button = Array.from(this.pinButtons).find(b => b.getAttribute('data-number') === keyOrAction);
        } else {
            button = Array.from(this.pinButtons).find(b => b.getAttribute('data-action') === keyOrAction);
        }
