...
class FurryTagSearch {
    ...
    loadTagData() {
        ...
        try {
            ...
            logger.bootSubStep(`Loaded ${Object.keys(this.tagData).length} furry tags`);
        } catch (error) {
            logger.error('Error loading furry tag dataset:', error.message);
            this.tagData = {};
            this.searchIndex = {};
        }
    }
    ...
    loadSearchIndexes() {
        try {
            ...
            logger.bootSubStep(`Furry indexes: ${Object.keys(this.searchIndex).length} terms, ${Object.keys(this.wordIndex).length} words, ${Object.keys(this.prefixIndex).length} prefixes, ${Object.keys(this.suffixIndex).length} suffixes`);
        } catch (error) {
            logger.warn('Error loading furry indexes, rebuilding:', error.message);
            this.buildSearchIndex();
            this.saveSearchIndexes();
        }
    }
    ...
    saveSearchIndexes() {
        try {
            ...
            logger.error('Error saving furry indexes:', error.message);
        }
    }
    ...
    findSeriesMatches(searchTerm) {
        ...
        Object.keys(this.tagData).forEach(tagName => {
            ...
            if (tagInfo.e_category === 'character') {
