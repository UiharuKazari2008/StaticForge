...
function getApocryphaData() {
    try {
        ...
        console.error('[apocrypha] failed to read/parse current.json:', e);
    }
    return null;
}
...
function renderApocrypha({ title, isGrimoire }) {
    ...
    const officialCount = counts.official !== undefined ? escapeHtml(counts.official) : '—';
    const unofficialCount = counts.unofficial !== undefined ? escapeHtml(counts.unofficial) : '—';
    const enshutsukaCount = counts.enshutsuka !== undefined ? escapeHtml(counts.enshutsuka) : '—';
    ...
    let enshutsukaHtml = '';
    if (data.enshutsuka && Array.isArray(data.enshutsuka)) {
        if (isGrimoire) {
    ...
    let billboardHtml = '';
    if (data.billboard && Array.isArray(data.billboard)) {
        billboardHtml = data.billboard.map(item => {
            const isHero = item.kind === 'hero' ? ' hero' : '';
            const imgHtml = (item.src && item.src.startsWith('http'))
    ...
    // Add fake ads back to billboard
    billboardHtml += `
                <div class="ad-tile">
        ...
        <div class="zine">
            ...
            <div class="content">
                <article class="main-col">
                    ...
                    <div class="article">
                        <span class="article-kicker">${kickerStr}</span>
