...
const API_KEY = secureConfig.openai?.apiKey || process.env.OPENAI_API_KEY;

if (!API_KEY) {
    console.warn('⚠️ OpenAI API key not found in secure.config.json or environment variables');
}
...
function createPersonaChatSession(sessionData, personaSettings, systemPrompt) {
    ...
    const conversationContext = promptManager.prepareConversationContext(sessionData.id, 'chatgpt');
    ...
    return {
        ...
        model: sessionData.model || "gpt-5-nano",
...
async function establishPersona(chat, personaImage, userPrompt, viewerAvatar) {
    try {
        ...
        if (chat.model === "gpt-5-nano" || chat.model === "gpt-5") {
            apiConfig.reasoning_effort = chat.thoughtLevel || "minimal";
            // gpt-5 models don't support custom temperature
        } else if (chat.model === "o4-high" || chat.model === "gpt-o4") {
            // o4 models support temperature
...
async function continueConversationWithContext(chat) {
    try {
        ...
        // Add model-specific parameters
        if (chat.model === "gpt-5-nano" || chat.model === "gpt-5") {
            // GPT-5 models support temperature
            if (chat.temperature !== undefined) {
                apiConfig.temperature = chat.temperature;
            }
        } else {
