...
async function enrichDynamicGenerationForMcp(globalResources, bindKey, dynamicGeneration, requestIP) {
    const settings = (dynamicGeneration && typeof dynamicGeneration === 'object' && !Array.isArray(dynamicGeneration))
        ? { ...dynamicGeneration }
        : null;
    const bound = bindKey ? getBoundRecord(globalResources, bindKey) : null;
    const boundIP = bound && bound.info ? bound.info.clientIP : null;
    const clientIP = boundIP || requestIP || null;
    let resolved = null;
...
async function getClientPhysics(globalResources, bindKey, overrides, requestIP) {
    ...
    const enriched = await enrichDynamicGenerationForMcp(globalResources, bound ? key : null, settings, requestIP);
